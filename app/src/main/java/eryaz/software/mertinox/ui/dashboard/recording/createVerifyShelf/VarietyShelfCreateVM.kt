package eryaz.software.mertinox.ui.dashboard.recording.createVerifyShelf
...
import eryaz.software.mertinox.data.models.remote.request.ProductShelfInsertRequest
import eryaz.software.mertinox.data.models.remote.request.ProductShelfUpdateRequest
...
class VarietyShelfCreateVM(private val repo: WorkActivityRepo) : BaseViewModel() {
    ...
    fun getShelfAddressFromApi() = executeInBackground(showProgressDialog = true) {
        repo.getShelfByCode(
            ...
            showError(
                ErrorDialogDto(
                    title = stringProvider.invoke(eryaz.software.mertinox.data.R.string.error),
                    message = stringProvider.invoke(eryaz.software.mertinox.data.R.string.msg_shelf_not_found)
                )
            )
        }
    }
